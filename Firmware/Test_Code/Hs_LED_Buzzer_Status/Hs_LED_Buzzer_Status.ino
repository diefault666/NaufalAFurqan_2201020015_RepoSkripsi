...
void updateStatusOutput(String status) {
  ...
  // TENANG: LED hijau menyala tetap
  if (status == "TENANG") {
    digitalWrite(LED_HIJAU, HIGH);
    digitalWrite(BUZZER_PIN, LOW);
  }

  // RENDAH: hijau kedip / kuning pelan
  ...
  // SEDANG: LED kuning + buzzer pelan
  ...
  // TINGGI ke atas: LED merah + buzzer cepat
  else if (
    status == "TINGGI" ||
...
void setup() {
  ...
  DEBUG_SERIAL.println("====================================");
  DEBUG_SERIAL.println("SMART BUOY - TEST C09");
  DEBUG_SERIAL.println("Hs + LED Status + Buzzer");
  DEBUG_SERIAL.println("SDA: GPIO17 | SCL: GPIO18");
  DEBUG_SERIAL.println("LED biru: GPIO1");
  DEBUG_SERIAL.println("LED hijau: GPIO4");
  DEBUG_SERIAL.println("LED kuning: GPIO5");
  DEBUG_SERIAL.println("LED merah: GPIO6");
  DEBUG_SERIAL.println("Buzzer: GPIO7");
