...
void resetHeaveIfStill(float heave_acc_ms2, float gx_dps, float gy_dps, float gz_dps) {
  ...
  // Jika diam sekitar 2 detik pada 50 Hz, velocity ditahan ke 0
  if (stillCounter > 50) {
    heave_velocity_ms = 0.0;
    heave_acc_filtered = 0.0;

    // Heave tidak langsung direset total, hanya ditarik pelan ke 0
    heave_m *= 0.98;
  }
}
...
void setup() {
  ...
  DEBUG_SERIAL.println("====================================");
  DEBUG_SERIAL.println("SMART BUOY - TEST C06");
  DEBUG_SERIAL.println("Heave Acceleration Integration");
  ...
  DEBUG_SERIAL.println("sample,dt,ax_g,ay_g,az_g,gx_dps,gy_dps,gz_dps,roll_deg,pitch_deg,acc_z_world_g,heave_acc_ms2,heave_acc_filtered,heave_velocity_ms,heave_m");
}
...
void loop() {
  ...
  // Deadband untuk mengurangi noise diam
  ...
  // Integrasi acceleration -> velocity
  heave_velocity_ms += heave_acc_filtered * dt;

  // Damping untuk menekan drift velocity
  heave_velocity_ms *= VELOCITY_DAMPING;

  // Integrasi velocity -> heave
  heave_m += heave_velocity_ms * dt;

  // Damping posisi
  heave_m *= HEAVE_DAMPING;

  // Deteksi diam dan redam velocity
  ...
  DEBUG_SERIAL.println(heave_m, 5);
