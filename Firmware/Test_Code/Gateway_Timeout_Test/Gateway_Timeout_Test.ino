...
void loop() {
  ...
  if (millis() - lastReceiveMillis > TIMEOUT_MS) {
    ...
    if (!timeoutPrinted) {
      DEBUG_SERIAL.println();
      DEBUG_SERIAL.println("TIMEOUT: Data LoRa tidak diterima lebih dari 5 detik.");
      DEBUG_SERIAL.println("Gateway masuk mode WAITING.");
      DEBUG_SERIAL.println("LED biru menyala terus.");
      timeoutPrinted = true;
    }
  } else {
    updateStatusOutput(latestStatus);
