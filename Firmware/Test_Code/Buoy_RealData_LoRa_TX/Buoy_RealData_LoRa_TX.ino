...
// Data global untuk dikirim LoRa
float latestRoll = 0.0;
float latestPitch = 0.0;
float latestHeaveFiltered = 0.0;
float latestHs = 0.0;
float latestTp = 2.80;
String latestStatus = "TENANG";
...
void showOledBoot() {
  ...
  display.setCursor(0, 0);
  display.println("SMART BUOY TX");
  display.println("D03 Real Data");
  display.println("OLED + LoRa");
  display.println();
...
void sendLoRaPayload() {
  ...
  dataNoCRC += timestamp;
  dataNoCRC += ",";
  dataNoCRC += String(latestHeaveFiltered, 3);
  dataNoCRC += ",";
  dataNoCRC += String(latestHs, 3);
  dataNoCRC += ",";
  dataNoCRC += String(latestTp, 2);
  dataNoCRC += ",";
  dataNoCRC += String(latestRoll, 2);
  dataNoCRC += ",";
  dataNoCRC += String(latestPitch, 2);
  dataNoCRC += ",";
  dataNoCRC += latestStatus;
  ...
  DEBUG_SERIAL.print("Sent real payload: ");
...
void setup() {
  ...
  DEBUG_SERIAL.println("====================================");
  DEBUG_SERIAL.println("SMART BUOY - D03 Real Data LoRa TX");
  DEBUG_SERIAL.println("BUOY UNIT");
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) {
    DEBUG_SERIAL.println("WARNING: OLED tidak terdeteksi.");
  } else {
  ...
  if (!LoRa.begin(LORA_FREQ)) {
    DEBUG_SERIAL.println("ERROR: LoRa init failed. Cek wiring LoRa.");
    while (true) {
      digitalWrite(LED_BIRU_BUOY, HIGH);
      delay(100);
      digitalWrite(LED_BIRU_BUOY, LOW);
  ...
  DEBUG_SERIAL.println("LoRa initialized.");
  DEBUG_SERIAL.println();

  DEBUG_SERIAL.println("sample,roll,pitch,heave,Hs,Tp,Status");
}
...
void loop() {
  ...
  if (nowMicros - lastSampleMicros >= SAMPLE_INTERVAL_US) {
    ...
    if (acc_norm < 0.5 || acc_norm > 2.5) {
      DEBUG_SERIAL.println("ERROR: Data akselerometer tidak valid.");
      return;
    }
    ...
    if (abs(heave_acc_ms2) > 5.0) {
      DEBUG_SERIAL.println("ERROR: Outlier heave acceleration.");
      return;
    }
    ...
    latestRoll = roll_deg;
    latestPitch = pitch_deg;
    latestHeaveFiltered = heave_filtered_m;
    latestHs = currentHs;
    latestStatus = status;

    DEBUG_SERIAL.print(sampleCounter); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.print(latestRoll, 2); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.print(latestPitch, 2); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.print(latestHeaveFiltered, 3); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.print(latestHs, 3); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.print(latestTp, 2); DEBUG_SERIAL.print(",");
    DEBUG_SERIAL.println(latestStatus);
